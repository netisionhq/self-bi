...
if ${USE_CACHE}; then
  ...
  uv pip install --trusted-host pypi.org --trusted-host files.pythonhosted.org "${ARGS[@]}"
else
  echo "Disabling pip cache..."
  uv pip install --trusted-host pypi.org --trusted-host files.pythonhosted.org --no-cache-dir "${ARGS[@]}"
fi
